...
async function getUserDocId(userId) {
  // 유저ID로 유저 문서 참조를 위해 문서 아이디 리턴
  const userDocIdList = await firestore().collection('User').where('id', '==', userId).get();

  if (userDocIdList.empty) {
    // 유저문서가 비어있을때 널값 리턴
...
// 매개변수: 게시물 시간
async function getPostDocId(date) {
  // 게시물의 docID 찾기
  const postDocIdList = await firestore().collection('Post').where('date', '==', date).get();

  if (postDocIdList.empty) {
    // 게시물 비어있을때 널값 리턴
...
async function getCommentsDocId(postDate, commentsDate) {
  ...
  const commentsDocIdList = await firestore()
    ...
    .doc(postDocId)
    .collection('Comments')
    .where('date', '==', commentsDate)
    .get();

  if (commentsDocIdList.empty) {
    // 댓글이 비어있을때 널값 리턴
