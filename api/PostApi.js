...
import { getImageUrl, imageUpload, metadataImage, setMetadata, deleteStorageImage } from './StorageImage'
...
async function findPostByTitle({title, start}) { // 게시물 제목으로 게시물 찾기
  const result = [];

  // limit부분을 start와 end로 바꿀 것
  const postList = await firestore().collection('Post')
  .where('title', '==', title).orderBy('date', 'desc').start(start).end(start+10).get(); // 게시물 검색 시 사용함으로 10개씩 받아옴
...
async function findPostList(userId) { // 맨 처음 메인페이지에다가 게시물 출력함
  ...
  for(const index of following) { // forEach는 await을 기다려주지 않아 for ...of문으로 대체했습니다.
    ...
    post.forEach(doc => { // 2차 반복문에서 팔로잉의 게시물을 전체 배열에 푸쉬
      if(doc.data().range != "Private") { // 공개범위가 비공개가 아닐 때
        postList.push(doc)
      }
    })
  }
...
// 매개변수: 게시물 시간
async function deletePost(postDate) { // 게시물 삭제
...
// 매개변수: 게시물 시간, 설정한 범위
async function postRangeUpdate({postDate, range}) { // 게시물 범위를 업데이트하는 함수
...
// 증분함수나 집계함수가 안먹힙니다....
async function likeUpdate({postDate, likeToken, like}) { // 좋아요 받는 함수
  const postDocId = await getPostDocId(postDate)
...
async function createComments({commentWriter, commentContent, postDate}) { // 댓글 작성자와 댓글 내용을 넣어 댓글 생성
  const postDocId = await getPostDocId(postDate);
...
async function readComments(postDate) {
  ...
  const comments = await firestore().collection('Post')
  .doc(postDocId).collection('Comments').orderBy('date', 'desc').get();
...
// 매개변수: 게시물 시간
async function readImages(postDate) { // 게시물의 이미지 불러오는 함수
  const postDocId = getPostDocId(postDate) // 게시물의 DocId 가져옴

  const result = [];

  const images = await firestore().collection('Post') // 이미지리스트 가져오기
  .doc(postDocId).collection('Images').get();
...
export {findPostById, findPostByTitle, findPostList, createPost, deletePost, updatePost,
        postRangeUpdate, likeUpdate, createComments, deleteComments, updateComments, readComments, 
        readImages, isMyPost, isMyComments}